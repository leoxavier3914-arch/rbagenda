...
- Dashboard: mostra gráfico dos agendamentos dos últimos 8 dias (linhas “Agendados” x “Confirmados”), cards de atividades (“Atividades”), tabela de clientes e tickets recentes. Usa Supabase para buscar `appointments` e `profiles` (role client) com fallback estático se falhar. Não há mais seleção de filial/branch nem KPIs antigos.
...
- Interação do shell admin: a sidebar recolhida mantém o mesmo padding/base do estado expandido e oculta metadados via transição de opacidade/visibilidade em vez de alterar tamanhos. Ícones ficam ancorados na mesma posição em ambos os estados e a lista de navegação tem `overflow: hidden` e rótulos `white-space: nowrap` para garantir colapso estritamente horizontal; como não há mais cabeçalho de usuário, não existe bloco extra antes do menu.
