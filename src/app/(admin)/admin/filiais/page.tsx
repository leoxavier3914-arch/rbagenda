...
export default function FiliaisPage() {
  ...
  const fetchBranches = useCallback(
    async (currentUserId: string, currentRole: AdminRole | "client" | null) => {
      ...
      const branchSelection = supabase
        .from("branches")
      ...
      if (isMaster) {
        const { data, error: branchesError } = await branchSelection;
        ...
        const [{ data: ownedBranches, error: ownedError }, assignedResult] = await Promise.all([
          branchSelection.eq("owner_id", currentUserId),
          assignmentIds.length > 0
            ? supabase.from("branches").select("id, name, timezone, owner_id, region, focus, status, staff_slots, created_at").in("id", assignmentIds)
