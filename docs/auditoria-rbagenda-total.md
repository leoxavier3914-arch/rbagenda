...
- **Shell de cliente**: `src/components/client/ClientPageLayout` expõe `ClientPageShell` (hero wrapper + `client-hero-ready`), `ClientSection` (centro + grid `page`/`stack`) e `ClientGlassPanel` (aplica classe `glass`/`label`). Usado em todas as rotas sensíveis; `ClientFullScreenLayout` injeta header/rodapé e é acionado pelo layout de `(client)`. `ClientPageShell` agora controla `force-motion` (com opt-out por hash `#nomotion`), centralizando o comportamento antes duplicado. `checkout` é exceção e recebe apenas `LavaLampProvider` pleno. `Procedimento` passou a usar explicitamente `ClientPageShell`/`ClientSection` via wrapper dedicado.
...
- **Signup**: rota `/(auth)/signup` segue fluxo padrão Supabase (registro simples) agora com `LavaLampProvider`, `ClientPageShell` + `ClientGlassPanel`, header padrão e CSS Module (`signup.module.css`) para alinhar visual ao login.
...
- **Glass**: classe global `.glass` + `.label` em `globals.css`; `ClientGlassPanel` envolve seções principais das páginas. `globals.css` mantém apenas tokens/shared layout; ajustes específicos (rodapés, espaçamentos finos) ficam em CSS Modules (`procedimento.module.css`, `agendamentos.module.css`, `meu-perfil.module.css`, `login.module.css`, `signup.module.css`, `suporte.module.css`, `regras.module.css`, `configuracoes.module.css`, `indice.module.css`).
...
- `/configuracoes` e `/indice`: aderiram ao shell padrão (`ClientPageShell` + `ClientSection`), guard de sessão e headers unificados (`ClientPageHeader`), com CSS Modules dedicados para espaçamento e grade.
- Força de animação: `ClientPageShell` agora aplica/remover `force-motion` com opt-out via hash, substituindo efeitos diretos em páginas individuais.
- Signup: recebe shell/lava + `ClientGlassPanel` e CSS Module, aproximando o visual do login sem alterar fluxo de registro.
- Organização: criado barrel de componentes em `agendamentos/@components`, `procedimento/@components` e `regras/@components`; `rules.module.css` renomeado para `regras.module.css`; novos módulos `indice` e ajustes em `configuracoes.module.css` centralizam estilos locais.
## Cheat sheet
