...
- Painel admin: reconstruído do zero no template Xtreme (topbar escura, sidebar lateral fechada mostrando só ícones, cards brancos). Agora só existe `/admin`, com gráfico de agendamentos (8 dias), lista de atividades, tabela de clientes e tickets recentes; não há tema alternável nem seletor de filial.
...
- Estrutura: reduzida a uma única rota `/admin` (`src/app/(admin)/admin/page.tsx`) apoiada no shell `AdminShell` + `AdminNav` (`adminShell.module.css`, `adminNav.module.css`). O layout replica o template Xtreme: topbar escura com busca/ícones, chip do usuário logado e breadcrumb; sidebar vira uma faixa fixa na esquerda mostrando só ícones quando fechada (sem header), expandindo ao clicar em qualquer ponto/ícone no desktop ou via hambúrguer (mobile) com backdrop. O usuário logado aparece no topo da barra lateral e no cabeçalho; não há LavaLamp nem provedores de tema.
- Temas e dependências removidos: `AdminThemeProvider`, `admin-theme.css`, contexto de filiais e todo o mini design system (`@components/ui`) foram eliminados. O visual é fixo em tons claros/azul sem presets; inputs/cartões são estilizados diretamente nos módulos CSS do shell e das páginas.
- Dashboard: mostra gráfico dos agendamentos dos últimos 8 dias (linhas “Agendados” x “Confirmados”), cards de atividades (“Atividades”), tabela de clientes e lista de tickets recentes. Usa Supabase para buscar `appointments` e `profiles` (role client) com fallback estático se falhar. Não há mais seleção de filial/branch nem KPIs antigos.
- Rotas e permissões: apenas `/admin` existe; rotas anteriores `/admin/agendamentos`, `/filiais`, `/servicos`, `/tipos`, `/clientes`, `/configuracoes`, `/suporte`, `/admin/adminsuper` e `/admin/adminmaster` foram removidas. `useAdminGuard` segue aceitando `admin`/`adminsuper`/`adminmaster`, mas todas as roles admin agora convergem para `/admin` (inclusive redirecionamentos do login e do menu).
- Responsividade: desktop ativa a partir de 900px (sidebar colada no topo com a mesma cor da topbar, exibindo o avatar mesmo recolhida e sem botão de toggle; clique fora/na navegação recolhe a lateral). Item ativo agora fica totalmente encaixado na faixa (sem deslocar nem sombra) e ícones inativos recebem fundo médio mais claro distinto do ativo. Abaixo de 900px vira drawer com backdrop. Ao cruzar o breakpoint, o menu móvel é fechado automaticamente para não cobrir conteúdo. As grades quebram para 1 coluna em telas estreitas, tabela possui scroll horizontal e os cards/tickets mantêm margens/gaps semelhantes ao template de referência.
- Navegação lateral: itens agora usam `box-sizing: border-box` para que padding e bordas não ultrapassem a faixa escura da sidebar, mantendo o destaque azul contido em estados colapsado/expandido no mobile e desktop.
- Interação do shell admin: a sidebar recolhida mantém o mesmo padding/base do estado expandido (incluindo altura do cabeçalho e avatar 44px) e oculta metadados via transição de opacidade/visibilidade em vez de alterar tamanhos. Ícones ficam ancorados na mesma posição em ambos os estados e a lista de navegação tem `overflow: hidden` e rótulos `white-space: nowrap` para garantir colapso estritamente horizontal. O cartão do usuário no topo tem altura mínima fixa e padding único nos dois estados, não força mais largura cheia e usa `white-space: nowrap` + `overflow: hidden` para revelar apenas o texto na horizontal sem mudar o alinhamento vertical ou o gap entre avatar e metadados.
- Nav items: padding/min-height/alinhamientos iguais para colapsado/expandido, ícones fixos em 38x38 sem encolher, labels/badges com `line-height: 1`/`align-self: center` para evitar aumento de altura ao revelar texto e colapso que apenas oculta texto/badges sem mexer no espaçamento.
