...
const normalizeReminder = (row: ReminderRow, customers: Map<string, CustomerProfile>): ReminderItem | null => {
  ...
  return {
    ...
    appointment: normalizeAppointment(row.appointments, customers),
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const { data: appointmentRows, error: appointmentsError } = scopedBranchId
          ? appointmentsQuery.eq("branch_id", scopedBranchId)
          : appointmentsQuery;
        ...
        const { data: reminderRows, error: remindersError } = scopedBranchId
  ...
  return (
    <div className={styles.dashboard}>
      <div className={styles.pageTop}>
        <div>
          <p className={styles.pageBreadcrumb}>Dashboard / Agendamentos</p>
          <h1 className={styles.pageTitle}>Agenda por filial</h1>
      ...
      <div className={styles.heroCards}>
        <section className={styles.card}>
          <header className={styles.cardHeader}>
            <div>
              <p className={styles.cardEyebrow}>Calendário</p>
        ...
        <section className={`${styles.card} ${styles.feedCard}`}>
          <header className={styles.cardHeader}>
            <div>
              <p className={styles.cardEyebrow}>Próximos agendamentos</p>
          ...
          <div className={styles.feedList}>
            ...
            {reminderFeed.map((item) => (
              <div key={item.id} className={styles.feedItem}>
                <span className={`${styles.feedIcon} ${styles[`tone-${STATUS_TONE[statusKey(item.status)] ?? "blue"}`]}`} aria-hidden />
                <div className={styles.feedCopy}>
                  <p className={styles.feedTitle}>{item.appointment.customerName}</p>
      ...
      <section className={styles.card}>
        <header className={styles.cardHeader}>
          <div>
            <p className={styles.cardEyebrow}>Agendamentos</p>
                ...
                <tr key={appt.id}>
                  <td>
                    <div className={styles.clientCell}>
                      ...
                      <div>
                        <p className={styles.clientName}>{appt.customerName}</p>
                        <p className={styles.clientEmail}>{appt.customerEmail ?? "—"}</p>
                      </div>
                    </div>
                  </td>
                  <td className={styles.valueCell}>{appt.serviceName}</td>
                  <td className={styles.mutedCell}>{appt.serviceTypeName ?? "—"}</td>
                  <td className={styles.mutedCell}>{appt.techniqueName ?? "—"}</td>
      ...
      <section className={styles.card}>
        <header className={styles.cardHeader}>
          <div>
            <p className={styles.cardEyebrow}>Métricas</p>
        ...
        {loading ? <div className={styles.loading}>Carregando agendamentos…</div> : null}
      </section>
    </div>
  );
}
