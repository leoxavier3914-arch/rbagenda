...
- **Agendamentos (`/agendamentos`):** página client-side lista agendamentos do Supabase, normaliza status, pagina 5 itens, permite filtrar (ativos/pendentes/cancelados/concluídos), recalcula disponibilidade com `buildAvailabilityData` para reagendamento, suporta cancelamento e pagamento de sinal via Stripe (usando `stripePromise`). Usa componentes `ClientPageShell`, `ClientSection`, `ClientGlassPanel`, `ClientPageHeader` e ícone `LashIcon`.
