...
- Filiais (`/admin/filiais`): página exclusiva de super e master para criar filiais, listar apenas as criadas/atribuídas (super) ou todas (master), vincular/desvincular administradores/staff e visualizar métricas locais (membros ativos por nível, criador, ocupação) com gráfico rápido de agendados x equipe ativa. O formulário cria filiais locais e destaca a necessidade de persistência futura.
