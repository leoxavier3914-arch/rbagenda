...
- Estrutura: `layout.tsx` aplica `AdminThemeProvider` (temas Light/Dark/Romeike/White Label persistidos em `localStorage`) e `AdminShell`. O lava foi removido do admin; o shell usa sidebar fixa no desktop e drawer no mobile, com seletor de filial na lateral, perfil/resumo no rodapé e topbar com pílulas de filial/role. Conteúdo renderiza dentro de um frame (`contentFrame`) mantendo as rotas reais (`/admin/*`).
- Navegação: `@components/AdminNav` permanece com links para `/admin`, `/admin/agendamentos`, `/admin/filiais`, `/admin/servicos`, `/admin/tipos`, `/admin/clientes`, `/admin/configuracoes`, `/admin/suporte`; ativa por path completo ou prefixo. Drawer fecha ao navegar no mobile.
- Design system do admin: `@components/AdminUI.tsx` expõe `AdminCard`, `AdminStatCard`, `AdminToolbar`, `AdminTable`, `AdminFormSection` e foi aplicado no dashboard, nas listas (agendamentos/clientes) e na área de temas para padronizar cards, tabelas e toolbars.
- Home do painel (`/admin`): novo layout estilo dashboard com `AdminCard` + `AdminStatCard` e grade de módulos; mantém guarda de role `admin` com redirecionos definidos por `useAdminGuard`.
- Painéis estratégicos: `/admin/adminsuper` e `/admin/adminmaster` continuam compartilhando componente client-side; guardas de role preservados (admin → `/admin`, adminsuper → `/admin/adminsuper`, adminmaster → `/admin/adminmaster`, cliente → `/login`). Admin master mantém seções extras.
- Módulos operacionais: `/admin/agendamentos`, `/admin/filiais`, `/admin/servicos`, `/admin/tipos`, `/admin/clientes`, `/admin/configuracoes` usam `AdminOperationsContent` com nova toolbar de busca global, cards/tabelas modernizadas (agendamentos em `AdminTable`, stats em `AdminStatCard`) e preservam toda a lógica Supabase/fluxos existentes.
- Configurações: `/admin/configuracoes` agora expõe seleção de temas do admin (4 presets) e cards de estado (role atual, filial ativa, tema aplicado), sem alterar roles/RLS.
- Responsividade do painel: sidebar vira drawer no mobile; grids e forms seguem 1 coluna em telas pequenas com `min-width: 0` para evitar overflow (continua valendo nos cadastros de serviços/tipos). Suporte permanece como placeholder.
