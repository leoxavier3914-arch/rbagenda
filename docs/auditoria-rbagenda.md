...
### 2.4 `/login`
- **Layout/UX**: página usa `LavaLampProvider` + shell compartilhado (`ClientPageShell`/`ClientSection`/`ClientGlassPanel`) com cartão glass premium (borda branca sutil, blur e sombra difusa), logo “ROMEIKE BEAUTY” em pílula translúcida sobre o card, inputs em formato pílula com ícones (✉️ para e-mail, 🔒 para senha) e placeholders verde-acinzentados, link de recuperação centralizado, botão “Entrar” em degradê de verdes e CTA inferior “Criar conta” sublinhada.
