...
- Painel admin: shell remodelado para layout de dashboard (sidebar fixa/drawer, topbar sticky) sem o frame branco central. O `main` agora é fluido, usa o fundo do tema, padding responsivo e evita overflow horizontal; os cartões/tabelas definem as superfícies.
...
- Estrutura: segue em `src/app/(admin)`, agora com `layout.tsx` aplicando `AdminThemeProvider` + `AdminShell` (sem LavaLamp). Sidebar fixa no desktop e drawer no mobile, avatar/perfil no rodapé e botão rápido de tema. A topbar permanece sticky e o wrapper do painel está totalmente fluido em estilo dashboard: sem moldura branca, `main` ocupando toda a largura útil, padding responsivo, `min-width: 0` e `overflow-x: hidden` para evitar cortes. O fundo do shell usa o próprio background do tema e deixa que cards/tabelas definam as superfícies. Temas (Light, Dark, Romeike Beauty, White Label) continuam nas variáveis de `admin-theme.css` e persistem em `localStorage`.
- Design system do admin: componentes `AdminCard`, `AdminStatCard`, `AdminToolbar`, `AdminTable` e `AdminFormSection` (em `@components/ui`) padronizam cartões, KPIs, toolbars e tabelas das rotas do painel. O shell usa `adminShell.module.css` e o nav foi redesenhado (`adminNav.module.css`) com ícones/pills consistentes.
...
- Responsividade do painel: formularios/grids seguem 1 coluna no mobile e evitam overflow horizontal; tabela/listas usam hover/empty states novos. O drawer mobile fecha ao navegar e o `main` agora evita overflow horizontal mesmo sem a moldura anterior, mantendo padding de 16–28px.
