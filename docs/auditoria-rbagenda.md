...
- Interação do shell admin: a sidebar recolhida mantém o mesmo padding/base do estado expandido (incluindo altura do cabeçalho e avatar 44px) e oculta metadados via transição de opacidade/visibilidade em vez de alterar tamanhos. A lista de navegação tem `overflow: hidden` e rótulos `white-space: nowrap` para garantir colapso estritamente horizontal.
