...
export default function NewAppointmentExperience() {
    ...
    const loadCatalog = async () => {
      ...
      try {
        ...
        const normalized = (data ?? []).map((entry) => {
          ...
          const servicesRaw = assignments.flatMap(
            (assignment: ServiceTypeAssignment) => {
          ...
          const services = servicesRaw
            ...
            .map((svc) => {
              ...
              return {
                ...
                buffer_min:
                  buffer !== null ? Math.max(0, Math.round(buffer)) : null,
        ...
        setCatalogError(
          "Não foi possível carregar os serviços disponíveis. Tente novamente mais tarde.",
        );
      }
    };
    ...
    async function loadAvailability() {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from("appointments")
          .select(
            "id, scheduled_at, starts_at, ends_at, status, customer_id, services(buffer_min)",
          )
        ...
        if (isMounted) {
          setAvailabilityError(
            "Não foi possível carregar a disponibilidade. Tente novamente mais tarde.",
          );
          setAppointments([]);
        }
      } finally {
  ...
  useEffect(() => {
    ...
    if (
      selectedServiceId &&
      !activeServices.some((svc) => svc.id === selectedServiceId)
    ) {
      setSelectedServiceId(null);
    }
  ...
  const selectedService = useMemo(
    () =>
      selectedType?.services.find((svc) => svc.id === selectedServiceId) ??
      null,
    [selectedServiceId, selectedType],
  );
  ...
  const calendarHeaderDays = useMemo(() => {
    ...
    return Array.from(
      { length: 7 },
      (_, index) => labels[(startWeekday + index) % 7],
    );
  }, [month, year]);
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
      ...
      let status: "available" | "booked" | "full" | "mine" | "disabled" =
        "disabled";
      ...
      const isDisabled =
        !canInteract || isPast || status === "full" || status === "disabled";
  ...
  const slots = useMemo(() => {
    ...
    const template =
      availability.daySlots[selectedDate] ?? DEFAULT_SLOT_TEMPLATE;
    ...
    return template.filter((slotValue) => {
      ...
      const slotEnd = new Date(
        slotStart.getTime() + (durationMinutes + serviceBufferMinutes) * 60000,
      );
      if (slotEnd > closing) {
        return false;
      }
  ...
  const hasMetRequirements = Boolean(
    selectedDate &&
      selectedSlot &&
      selectedService &&
      selectedType &&
  ...
  async function ensureAppointment(session: Session) {
    ...
    if (!appointmentId) {
      throw new Error(
        "Resposta inválida ao criar o agendamento. Tente novamente.",
      );
    }
    ...
    <div className={styles.page}>
      <header className={styles.hero}>
        ...
        <p className={styles.subtitle}>
          Escolha a técnica e o tipo de serviço, além da data e horário. O
          preço, tempo e sinal atualizam automaticamente.
        </p>
      </header>
      ...
      <FlowShell className={styles.shellExtras}>
        <div className={styles.shellSurface}>
          <section
            ...
            id="tipo-card"
          >
            <div className={`${styles.label} ${styles.labelCentered}`}>
              Tipo
            </div>
            {catalogError && (
            ...
            {catalogStatus === "ready" && availableTypes.length === 0 && (
              <div className={styles.meta}>
                Nenhum tipo de serviço disponível no momento.
              </div>
            )}
            {catalogStatus === "ready" && availableTypes.length > 0 && (
              <div
            ...
            <>
                    ...
                    <div className={styles.optRow}>
                      <div className={styles.left}>
                        ...
                        <div>
                          ...
                          <div className={styles.meta}>
                            Isolamento e cola adequada para durabilidade
                          </div>
                        </div>
                      </div>
                    ...
                    <div className={styles.optRow}>
                      <div className={styles.left}>
                        ...
                        <div>
                          ...
                          <div className={styles.meta}>
                            {minutesToText(computed.durationMinutes)}
                          </div>
                        </div>
                      </div>
                      <div className={styles.meta} />
              ...
              >
                  ...
                  {calendarDays.dayEntries.map(
                    ({
                      ...
                      <button
                        ...
                        data-state={state}
                        data-selected={
                          !isOutsideCurrentMonth && selectedDate === iso
                        }
                        data-outside-month={
                          isOutsideCurrentMonth ? "true" : "false"
                        }
                ...
                <div className={styles.slots}>
                  {availabilityError ? (
                      ...
                      slots.map((slotValue) => {
                        ...
                        return (
                          <button
                            ...
                            disabled={disabled}
                            onClick={() =>
                              handleSlotSelect(slotValue, disabled)
                            }
                          >
                            {slotValue}
                          </button>
                    ...
                    <div className={styles.meta}>
                      Selecione um dia disponível para ver horários.
                    </div>
                  )}
                </div>
              </section>
              ...
              >
                ...
                <ul className={styles.rules}>
                  ...
                  <li>
                    Reaplicação: quando não atende às regras de manutenção.
                  </li>
          ...
          <div className={styles.summaryInner}>
            <div className={styles.grow}>
              ...
              <div className={styles.price}>
                R$ {toBRLCurrency(computed.total)}
              </div>
              <div className={styles.meta}>
                Sinal: R$ {toBRLCurrency(computed.deposit)}
              </div>
        ...
        <div className={styles.modalContent} role="document">
          ...
          <div className={styles.modalBody}>
            ...
            <div className={styles.modalLine}>
              <span>Data</span>
              <strong>
                {selectedDate ? formatIsoDateToBR(selectedDate) : "—"}
              </strong>
            </div>
            <div className={styles.modalLine}>
              <span>Horário</span>
            ...
            {summaryError && (
              <div className={styles.modalError}>{summaryError}</div>
            )}
          </div>
      ...
      >
        ...
        <div className={styles.noticeContent} role="document">
          ...
          <p className={styles.noticeText}>
            O <strong>sinal deve ser pago em até 2 horas</strong> para garantir
            sua reserva. Após esse prazo, o horário será{" "}
            <strong>cancelado automaticamente</strong> e liberado para novos
            agendamentos.
          </p>
          {payLaterError && (
            <div className={styles.noticeError}>{payLaterError}</div>
          )}
          <button
