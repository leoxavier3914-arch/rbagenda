...
- Painel admin: shell e UI seguem o visual Xtreme (topbar em gradiente azul/violeta, sidebar escura com indicador lateral, cards com faixa superior). No desktop, a sidebar fixa ocupa a coluna esquerda e empurra o conteúdo; no mobile, vira drawer em altura cheia com backdrop e fechamento por clique fora/ESC. O root cobre 100dvh com `--admin-body`/`--admin-content-bg` e padding responsivo sem moldura branca; inputs/selects/cards forçam largura total.
...
- Estrutura: segue em `src/app/(admin)`, com `layout.tsx` aplicando `AdminThemeProvider` + `AdminShell` (sem LavaLamp). O shell agora replica o estilo Xtreme (topbar em gradiente azul/violeta, breadcrumbs e avatar claros, sidebar dark com indicador lateral) mantendo grid com topbar sticky. No desktop a sidebar fixa ocupa a coluna esquerda e empurra o conteúdo; no mobile vira drawer/overlay em altura cheia com backdrop + ESC/clique fora e padding interno que evita sobrepor a topbar. O root cobre 100dvh com `--admin-body`/`--admin-content-bg` sem moldura branca, `main` full-width, `min-width: 0` e `overflow-x: hidden`. Temas (Light, Dark, Romeike Beauty, White Label) seguem em `admin-theme.css` com novos tokens (`--admin-body`, `--admin-content-bg`, `--admin-header-gradient`, `--admin-sidebar-*`, `--admin-card-glow`) e persistência em `localStorage`.
- Design system do admin: componentes `AdminCard`, `AdminStatCard`, `AdminToolbar`, `AdminTable` e `AdminFormSection` (em `@components/ui`) padronizam cartões, KPIs, toolbars e tabelas das rotas do painel. Cartões agora trazem faixa superior no gradiente do shell, sombras mais fortes e tabelas/toolbars com fundos em gradiente leve; o nav (`adminNav.module.css`) usa indicador lateral animado e hover ativo no fundo escuro da sidebar.
- Home do painel: `/admin` virou dashboard operacional (apenas role `admin`), exibindo KPIs, agendamentos do dia, clientes recentes e atalhos de navegação, respeitando a seleção de filial (guard de branch) e os counts do Supabase. Não usa mais fundo glass/lava.
- Configurações: `/admin/configuracoes` ganhou seção de temas (presets do `AdminThemeProvider`) e ações rápidas de sessão; o tema selecionado vale apenas para o admin.
- Módulos operacionais: `/admin/agendamentos`, `/admin/filiais`, `/admin/servicos`, `/admin/tipos`, `/admin/clientes`, `/admin/configuracoes` continuam centralizados em `@components/AdminOperationsContent`, agora com cards/tabelas/toolbars do mini design system e sem fundo lava. Seções de filiais/serviços/tipos mantêm regras de permissão (owner/admin master) e exigência de filial quando aplicável.
- Responsividade do painel: formularios/grids seguem 1 coluna no mobile e evitam overflow horizontal; tabelas/listas usam hover/empty states novos. O drawer mobile fecha ao navegar, bloqueia scroll do body e o conteúdo ocupa 100% da largura, sem moldura ou centralização artificial, inclusive para inputs/selects/cards.
- Proteção: `useAdminGuard` mantém redirects por role (adminsuper → `/admin/adminsuper`, adminmaster → `/admin/adminmaster`, admin → `/admin`, clientes → `/login`). Painéis master/super continuam validados e usam o mesmo estilo novo.
