...
- Estrutura: tudo em `src/app/(admin)` com `layout.tsx` aplicando `LavaLampProvider` + `AdminShell`. Criamos três grupos de rotas (sem mudar as URLs): `(panel-admin)` atende `/admin`, `(panel-super)` atende `/admin/adminsuper` e `(panel-master)` atende `/admin/adminmaster`. Cada página tem `page.tsx` fino chamando um panel local em `@components` da própria rota.
- Proteção: `PanelGuard` (novo em `@components`) encapsula `useAdminGuard` com `allowedRoles` + `fallbackRedirects` para direcionar para o painel correto: admin → `/admin`, adminsuper → `/admin/adminsuper`, adminmaster → `/admin/adminmaster`, demais/unauthenticated → `/login`. `useCurrentUserId` centraliza leitura de sessão Supabase para os painéis super/master.
- Navegação: `AdminNav` agora é sensível à role. Admin vê `/admin`, `/admin/filiais`, `/admin/agendamentos`, `/admin/clientes`; adminsuper vê `/admin/adminsuper`, `/admin/adminsuper/filiais`, `/admin/adminsuper/admins`, `/admin/adminsuper/agendamentos`, `/admin/adminsuper/clientes`; adminmaster vê `/admin/adminmaster`, `/admin/adminmaster/filiais`, `/admin/adminmaster/supers`, `/admin/adminmaster/auditoria`. Labels seguem “Admin”, “Admin super”, “Admin master”.
- Painel Admin: páginas modulares (painéis locais) carregam apenas dados da filial ativa via `AdminBranchContext` — dashboard com métricas básicas, edição de filial (nome/fuso), agendamentos filtrados e lista de clientes da filial. Sem owners/branch_admins aqui.
- Painel Admin super: rotas próprias para resumo, filiais onde `owner_id` = usuário logado, vínculo de admins via `branch_admins`, clientes e agendamentos das filiais dele. Cada página consulta apenas o necessário (branches, profiles ou appointments) e não depende de hub central.
- Painel Admin master: rotas para resumo global, gestão de filiais (owner adminsuper + branch_admins), gestão de supers (RPC `set_profile_role`) e auditoria placeholder. Queries focadas em branches/profiles/appointments; sem `AdminOperationsContent`.
- Infra compartilhada removida: `AdminOperationsContent` foi deletado; cada módulo tem painel local em `@components` dentro de sua rota. `AdminBranchContext` continua servindo a seleção de filiais para admin/adminsuper e permite `no_branch` para master.
- Suporte: `/admin/suporte` permanece como módulo isolado (usa apenas `AdminBranchContext`).
