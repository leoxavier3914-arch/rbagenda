...
- Painel admin: shell agora usa grid com topbar e sidebar como um bloco único (mesmo tema/superfície). No desktop, a sidebar fixa ocupa a coluna esquerda e empurra o conteúdo; no mobile, vira drawer abaixo da topbar com backdrop e fechamento por clique fora/ESC. O root recebe o `--admin-bg` e ocupa 100dvh/100% sem moldura, sombras ou bordas externas, com `main` full-width, `min-width: 0`, `overflow-x: hidden` e inputs/selects/cards forçando largura total.
...
- Estrutura: segue em `src/app/(admin)`, com `layout.tsx` aplicando `AdminThemeProvider` + `AdminShell` (sem LavaLamp). O shell usa grid com topbar sticky e sidebar na mesma superfície/tema: topbar ocupa toda a largura e a sidebar aparece logo abaixo, sem parecer blocos separados. No desktop a sidebar fixa ocupa a coluna esquerda e empurra o conteúdo; no mobile ela vira drawer/overlay abaixo da topbar (backdrop, ESC/clique fora, sem scroll do body). Sem moldura branca: root em `--admin-bg` 100dvh/100%, `main` full-width com padding responsivo, `min-width: 0` e `overflow-x: hidden`. Temas (Light, Dark, Romeike Beauty, White Label) continuam nas variáveis de `admin-theme.css` e persistem em `localStorage`.
