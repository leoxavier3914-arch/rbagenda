...
- Shell e sidebar: `adminShell.module.css` agora usa grid com coluna fixa em `var(--sidebar-expanded)` + `1fr`, preservando o espaço do conteúdo mesmo com a lateral recolhida. O `<aside>` mantém largura cheia e, no estado colapsado, só oculta o conteúdo via `transform`/padding (sem alterar `width`), deixando o main estável e evitando “pulos” ao expandir/recolher. No mobile o overlay continua fechando com clique fora.
