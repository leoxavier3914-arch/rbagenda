...
- Painel admin: reconstruído do zero no template Xtreme (topbar escura, sidebar lateral fechada mostrando só ícones, cards brancos). Agora existem `/admin` e `/admin/agendamentos`: o primeiro mantém o gráfico de 8 dias, atividades, clientes e tickets, enquanto o segundo traz calendário por filial, lista de lembretes/próximos atendimentos, tabela cronológica e métricas de serviços/tipos. Não há tema alternável; a filial usada é a atribuída (admins) ou a selecionável futuramente (super/master).
...
- Agendamentos (`/admin/agendamentos`): calendário mensal com eventos por filial atribuída (admins) ou selecionável futuramente (super/master), lista de lembretes/próximos atendimentos (reminders ou fallback dos próximos appointments), tabela cronológica (cliente, nome, tipo, técnica, data/hora) ordenada a partir de agora e que oculta cancelados/concluídos, além de cartão de métricas (serviços/tipos com filtros 7d/30d/ano + barras mensais). Dados filtram por `branch_id` quando houver vínculo; sem seleção, super/master podem ver todas as filiais permitidas pela RLS.
