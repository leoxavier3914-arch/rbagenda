...
- Painel admin: shell ajustado para header único conectado à sidebar. No desktop, sidebar fixa empurra o conteúdo em duas colunas; no mobile, vira drawer com backdrop, trava scroll do body e fecha por clique fora/ESC. Sem moldura branca e sem max-width reduzido: o `main` ocupa toda a largura útil, mantém padding responsivo e previne overflow horizontal; inputs/selects/cards forçam largura total.
...
- Estrutura: segue em `src/app/(admin)`, com `layout.tsx` aplicando `AdminThemeProvider` + `AdminShell` (sem LavaLamp). O shell agora tem header sticky conectado visualmente à sidebar: à esquerda ficam brand/breadcrumbs e à direita o seletor de filial, toggle de tema e pill de usuário/avatar. No desktop a sidebar fica fixa, empurra o conteúdo (2 colunas) e respeita o offset do header; no mobile ela vira drawer/overlay com backdrop, bloqueia o scroll do body, fecha em clique fora/ESC e mantém close dedicado. Sem moldura branca: `main` ocupa toda a largura útil com padding responsivo, `min-width: 0` e `overflow-x: hidden` para evitar cortes. Temas (Light, Dark, Romeike Beauty, White Label) continuam nas variáveis de `admin-theme.css` e persistem em `localStorage`.
- Design system do admin: componentes `AdminCard`, `AdminStatCard`, `AdminToolbar`, `AdminTable` e `AdminFormSection` (em `@components/ui`) padronizam cartões, KPIs, toolbars e tabelas das rotas do painel. O shell usa `adminShell.module.css` e o nav (`adminNav.module.css`) mantém ícones/pills consistentes.
