...
- Fundo animado controlado por `LavaLampProvider` e variáveis CSS globais (`--bg-*`, `--inner-*`, `--glass`, `--glass-stroke`, `--card-stroke`, `--dark`, `--light`, `--lava-alpha-*`); `refreshPalette` redesenha blobs. Classes globais (`client-hero-wrapper`, `page`, `stack`, `glass`, `label`) estão em `globals.css`.
...
- **Layout e UX**: `ProcedimentoWrapper` aplica hero + page; seções em vidro com ícones `LashIcon`; legenda de calendário (available/booked/full/mine/disabled); barra de resumo fixa. Respeita `prefers-reduced-motion` e força `force-motion` salvo se removido manualmente.
...
- **Organização**: fluxo dividido em componentes; lógica de disponibilidade encapsulada no hook. Mantém padrão estrutural do shell.
...
- `/procedimento`: modularização completa com hook compartilhado de disponibilidade e fallback de buffer/timezone.
